...
def D(inputs, name, reuse=False):
    with tf.variable_scope(name, reuse=reuse):
        l1 = dense(inputs, [784, 128], name="relu1", act_fun=tf.nn.relu)
        # l2 = dense(l1, [512, 512], name="relu2", act_fun=tf.nn.relu)
        # l3 = dense(l2, [512, 512], name="relu3", act_fun=tf.nn.relu)
        y = dense(l1, [128, 1], name="output")
        return y


def G(inputs, name, reuse=False):
    with tf.variable_scope(name, reuse=reuse):
        l1 = dense(inputs, [100, 128], name="relu1", act_fun=tf.nn.relu)
        # l2 = dense(l1, [512, 512], name="relu2", act_fun=tf.nn.relu)
        # l3 = dense(l2, [512, 512], name="relu3", act_fun=tf.nn.relu)
        y = dense(l1, [128, 784], name="output", bn=True, act_fun=tf.nn.sigmoid)
        return y
...
for step in range(100 * 1000):
    ...
    if step % 1000 == 999:
        ...
        Image.fromarray(image).save("out/Step_%d.jpg" % (step + 1))
        saver = tf.train.Saver()
